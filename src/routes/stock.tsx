...
function Page() {
  const { articles, addArticle, updateArticle, removeArticle } = useOperations();
  ...
  function launch(next: "add" | "edit" | "view", a?: Article) {
    setMode(next);
    setSelected(a ?? null);
    setForm(a ? { nom: a.nom, categorie: a.categorie, unite: a.unite, stockInitial: a.stockInitial, seuil: a.seuil, prixAchat: a.prixAchat, point: a.point, frequence: a.frequence } : emptyForm);
    setOpen(true);
  }
  function save() {
  ...
  return <AppShell title="Produits & Stocks" subtitle={`${articles.length} références · Stock J = Stock J-1 + Achats - Ventes - Prélèvements`} action={<Button onClick={() => launch("add")}><PackagePlus className="mr-2 h-4 w-4" />Ajouter un produit</Button>}>
      ...
      <div className="mt-5 overflow-x-auto">
        <Table>
          ...
          <TableBody>{paged.map((a) => <TableRow key={a.id}>
            ...
            <TableCell><Badge variant="outline">{a.frequence}</Badge></TableCell>
            <TableCell>{a.categorie}</TableCell>
            <TableCell>{a.unite}</TableCell>
            <TableCell>{a.stockInitial}</TableCell>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent>
      <DialogHeader><DialogTitle>{mode === "add" ? "Ajouter un produit" : mode === "edit" ? "Éditer le produit" : "Détails du produit"}</DialogTitle><DialogDescription>Les données sont mises à jour instantanément dans ce MVP.</DialogDescription></DialogHeader>
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="sm:col-span-2"><Label>Nom</Label><Input disabled={mode === "view"} value={form.nom} onChange={(e) => setForm({ ...form, nom: e.target.value })} /></div>
        <div><Label>Catégorie</Label><Select disabled={mode === "view"} value={form.categorie} onValueChange={(v) => setForm({ ...form, categorie: v as Article["categorie"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{categories.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
        <div><Label>Unité</Label><Input disabled={mode === "view"} value={form.unite} onChange={(e) => setForm({ ...form, unite: e.target.value })} /></div>
        <div><Label>Stock fixe</Label><Input disabled={mode === "view"} type="number" value={form.stockInitial} onChange={(e) => setForm({ ...form, stockInitial: Number(e.target.value) })} /></div>
        <div><Label>Seuil d'alerte</Label><Input disabled={mode === "view"} type="number" value={form.seuil} onChange={(e) => setForm({ ...form, seuil: Number(e.target.value) })} /></div>
        <div><Label>Prix d'achat (MAD)</Label><Input disabled={mode === "view"} type="number" value={form.prixAchat} onChange={(e) => setForm({ ...form, prixAchat: Number(e.target.value) })} /></div>
        <div><Label>Service</Label><Select disabled={mode === "view"} value={form.point} onValueChange={(v) => setForm({ ...form, point: v as Article["point"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["Bar", "Cuisine"].map((p) => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select></div>
        <div className="sm:col-span-2"><Label>Fréquence de prélèvement</Label><Select disabled={mode === "view"} value={form.frequence} onValueChange={(v) => setForm({ ...form, frequence: v as Frequence })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Quotidien">Quotidien</SelectItem><SelectItem value="Hebdomadaire">Hebdomadaire</SelectItem></SelectContent></Select></div>
      </div>
      <DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Fermer</Button>{mode !== "view" && <Button onClick={save}>Enregistrer</Button>}</DialogFooter>
